...
Here is an illustration of the meat consumption barplot depending whether the respondents ever had cancer or not.
```{r, warning = FALSE}
#Graph barplot
join_food_other_cancer %>% filter(!is.na(got_cancer)) %>% ggplot(aes(x = meat_cons, group = got_cancer, fill = as.factor(got_cancer))) + geom_bar() + labs(x = "meat consumption", y = "count", title = "Meat consumption barplot if ever got cancer or not") +  scale_fill_manual("Ever had cancer?", values  = c("0"  = "green2", "1"  = "red1")) + scale_x_continuous(breaks=seq(1,10))
```
We can see that the greater the frequency is, the greater the gap in barplot is.  
...
Here is an illustration of the dairy products consumption barplot depending whether the respondens ever had cancer or not.

```{r, warning = FALSE}
#Graph barplot
join_food_other_cancer %>% filter(!is.na(got_cancer)) %>% ggplot(aes(x = dairy_cons, group = got_cancer, fill = as.factor(got_cancer))) + geom_bar() + labs(x = "dairy products consumption", y = "count", title = "dairy products consumption if ever got cancer or not") +  scale_fill_manual("Ever had cancer?", values  = c("0"  = "green2", "1"  = "red1")) + scale_x_continuous(breaks=seq(1,10))
```
We can see that the greater the frequency is, the smaller the gap in barplot is between the two groups.  
...
Here is an illustration of the vegetable consumption barplot depending whether respondents ever had cancer or not.

```{r, warning = FALSE}
#Graph barplot
join_food_other_cancer %>% filter(!is.na(got_cancer)) %>% ggplot(aes(x = vege_cons, group = got_cancer, fill = as.factor(got_cancer))) + geom_bar() + labs(x = "Vegetable consumption", y = "count", title = "Vegetable consumption barplot if ever got cancer or not") +  scale_fill_manual("Ever had cancer?", values  = c("0"  = "green2", "1"  = "red1")) + scale_x_continuous(breaks=seq(1,10))
```
...
Here is an illustration of the fruit consumption barplot depending whether respondents ever had cancer or not.

```{r, warning = FALSE}
#Graph barplot
join_food_other_cancer %>% filter(!is.na(got_cancer)) %>% ggplot(aes(x = fruit_cons, group = got_cancer, fill = as.factor(got_cancer))) + geom_bar() + labs(x = "fruit consumption", y = "count", title = "Fruit consumption barplot if ever got cancer or not") +  scale_fill_manual("Ever had cancer?", values  = c("0"  = "green2", "1"  = "red1")) + scale_x_continuous(breaks = c(0, 2, 4, 6, 8, 10))
```
We can see that the greater the frequency is, the smaller the gap in barplot is between the two groups.  
...
Here is an illustration of the diet barplot depending whether respondents ever had cancer or not.
```{r, warning = FALSE}
#Graph barplot
Dietary_cancer %>% filter(!is.na(got_cancer)&!is.na(diet)) %>% ggplot(aes(x = diet, group = got_cancer, fill = as.factor(got_cancer))) + geom_bar() + labs(x = "diet", y = "count", title = "Diet consumption if ever got cancer or not") +  scale_fill_manual("Ever had cancer?", values  = c("0"  = "green2", "1"  = "red1"))
```
We can see that for excellent and very good diet the count is higher for the group who got cancer, then the count is relatively higher for the group who never got cancer.
...
We build a complete model including all the variables we explored previously, including those that were not significant at first, and then using the AIC method for variable selection.  

```{r}
model_full <- glm(got_cancer ~ age + gender + income + avg_physical_activity + avg_alcohol + cigarets_per_day + meat_cons + dairy_cons + vege_cons + fruit_cons + diet, family = binomial(link = 'logit'), data = join_food_other_cancer %>% drop_na())
...
```{r}
model_full_aic <- step(model_full)
...
McFadden's pseudo rsquared is equal to 0.0633 which means that only 6.33% of the variation in cancer is explained by this model, which is unfortunately very poor.
